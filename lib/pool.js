...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function OutputPoolInfo() {
        ...
        let buf = Buffer.from(_this.options.poolHex, "hex");
    ...
    function SetupPeer() {
        ...
        if (options.testnet && !options.coin.peerMagicTestnet) {
            ...
            emitErrorLog('p2p cannot be enabled without peerMagic set in coin configuration: ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port );
            return;
        }
        ...
        if (!process.env.forkId || process.env.forkId === '0'){
            emitLog('p2p connection successful ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port);
        }
        }).on('connectionRejected', function () {
            emitErrorLog('p2p connection failed - likely incorrect p2p magic value ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port );
        }).on('disconnected', function () {
         if (!process.env.forkId || process.env.forkId === '0'){
            emitWarningLog('p2p peer node disconnected - attempting reconnection...');
         }
        }).on('connectionFailed', function (e) {
            emitErrorLog('p2p connection failed - likely incorrect host or port ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port + " msg: " + JSON.stringify(e));
        }).on('socketError', function (e) {

console.log(" Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port  + ' '  + JSON.stringify(e));
            emitErrorLog('p2p had a socket error ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port);
        }).on('error', function (msg) {
            emitWarningLog('p2p had an error ' + " Enabled: " + options.p2p.enabled + " Peermagic: " + options.coin.peerMagic  + " Host: " + options.p2p.host + " Port: " + options.p2p.port + "msg: " + msg);
...
console.log('Node Type: ' + rpcCommand + ' Arguments: ' + rpcArgs);
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            if (!process.env.forkId || process.env.forkId === '0') {
            emitLog('No new blocks for Height: ' + _this.jobManager.currentJob.rpcData.height + ' Timeout: ' + options.jobRebroadcastTimeout + ' seconds, rebroadcasting work');
            }
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                emitLog('Connection from ' + client.getLabel() + ': ' + fullMessage.method);
    ...
    function SetupBlockPolling() {
        if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
           if (!process.env.forkId || process.env.forkId === '0') {
            emitLog('Block template polling has been disabled. Using P2P Notifications');
           }
            return;
        }
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function (error, result, foundNewBlock) {
                if (foundNewBlock)
                        emitLog( _this.jobManager.currentJob.rpcData.height + ' New Block notification via RPC');
//                    emitLog('Block notification via RPC polling');
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        let blockheight = parseInt(JSON.parse(_this.jobManager.currentJob.rpcData.height))
        blockheight = parseInt(blockheight);
        let nextblock = 0
        nextblock = blockheight + 1
        if (!process.env.forkId || process.env.forkId === '0') {
        emitLog('Block Notify: ' + parseInt(_this.jobManager.currentJob.rpcData.height) + ' via ' + sourceTrigger + ', Working next Block: '+ nextblock);
        }
        if (typeof(_this.jobManager) !== 'undefined' && typeof(_this.jobManager.currentJob) !== 'undefined' && typeof(_this.jobManager.currentJob.rpcData.previousblockhash) !== 'undefined' && blockHash !== _this.jobManager.currentJob.rpcData.previousblockhash) {
            GetBlockTemplate(function (error, result) {
                if (error)
                        if (!process.env.forkId || process.env.forkId === '0') {
                                emitErrorLog('Block notify error getting block: ' + _this.jobManager.currentJob.rpcData.height + ' template for ' + options.coin.name);
                        }
            })
        }
    };
        ...
        function hex2ascii(hexdata){
                hexdata.reverse;
                let buf = Buffer.from(hexdata, "hex");
                let asciiData = buf.toString("utf8");
                return asciiData.replace(/[^\x20-\x7E]/g, '');
        }
